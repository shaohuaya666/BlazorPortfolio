...
/// <summary>
/// 作品集数据库上下文。
/// 通过反射自动发现所有实现 IEntity 的实体类并注册，无需手动维护 DbSet 属性。
/// 查询时使用 context.Set&lt;T&gt;() 替代属性访问。
/// </summary>
public class PortfolioDbContext : DbContext
{
    ...
    /// <summary>
    /// 自动配置所有实体映射。
    /// 反射扫描当前程序集中实现了 IEntity 接口的类，自动注册到模型构建器。
    /// </summary>
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // 1. 反射自动发现并注册所有实体类型
        AutoRegisterEntities(modelBuilder);

        // 2. 配置实体间的关系（HasMany/WithOne/ForeignKey/Cascade）
    ...
    /// <summary>
    /// 扫描当前程序集，找出所有实现 IEntity 的非抽象类并注册到 modelBuilder。
    /// 表名与 MaxLength 约束由实体类上的 [Table] / [MaxLength] 数据注解驱动，
    /// EF Core 原生支持，无需在此重复配置。
    /// </summary>
    private static void AutoRegisterEntities(ModelBuilder modelBuilder)
    {
    ...
    /// <summary>
    /// 配置导航属性关系及级联删除策略。
    /// 这些关系语义无法通过数据注解完整表达，保留 Fluent API 配置。
    /// </summary>
    private static void ConfigureRelationships(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<CompactProject>()
            .HasMany(e => e.Skills)
            .WithOne(s => s.Project)
